...
#define VPU_BAUD	115200

extern "C" {__EXPORT int vpu_main(int argc, char *argv[]); }
...
int vpu_dev_init(void);

int vpu_dev_init(void)
{
   int baud_speed = B115200;

   uart_fd = open(VPU_SERIAL_PORT, O_RDWR | O_NOCTTY);

   if(uart_fd < 0)
   {
	FMX10_ERR("VPU: failed to open serial port: %s err: %d",VPU_SERIAL_PORT,errno);
	return -1;
   }
...
void task_main(int argc, char *argv[])
{
   if(vpu_dev_init() < 0)
   {
	FMX10_ERR("vpu uart init failed!");
	return;
   }
...
int start(void)
{
    ...
    _task_handle = fmx10_task_spawn_cmd("fm_vpu_main",
					SCHED_DEFAULT,
...
void usage(void)
{
    FMX10_INFO("Usage: vpu {start|info|stop}");
}
}
