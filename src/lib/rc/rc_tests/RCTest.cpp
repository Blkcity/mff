...
bool RCTest::dsmTest(const char *filepath, unsigned expected_chancount, unsigned expected_dropcount, unsigned chan0)
{
	...
	ut_test(fp != nullptr);
	//FMX10_INFO("loading data from: %s", filepath);
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		if (result) {
			...
			//FMX10_INFO("decoded packet with %d channels and %s encoding:", num_values, (dsm_11_bit) ? "11 bit" : "10 bit");

			for (unsigned i = 0; i < num_values; i++) {
				//FMX10_INFO("chan #%u:\t%d", i, (int)rc_values[i]);
			}
		}

		if (last_drop != (dsm_frame_drops)) {
			FMX10_INFO("frame dropped, now #%d", (dsm_frame_drops));
	...
	ut_test(ret == EOF);
	FMX10_INFO("drop: %d", (int)last_drop);
...
bool RCTest::sbus2Test(void)
{
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		if (last_drop != (sbus_frame_drops + sbus_frame_resets)) {
			FMX10_WARN("frame dropped, now #%d", (sbus_frame_drops + sbus_frame_resets));
...
bool RCTest::sumdTest(void)
{
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		if (!sumd_decode(b, &rssi, &rx_count, &channel_count, channels, 32)) {
			//FMX10_INFO("decoded: %u channels (converted to PPM range)", (unsigned)channel_count);

			for (unsigned i = 0; i < channel_count; i++) {
				//int16_t val = channels[i];
				//FMX10_INFO("channel %u: %d 0x%03X", i, static_cast<int>(val), static_cast<int>(val));
			}
		}
	}
